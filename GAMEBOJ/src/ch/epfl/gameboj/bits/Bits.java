package ch.epfl.gameboj.bits;

import static ch.epfl.gameboj.Preconditions.checkArgument;
import static ch.epfl.gameboj.Preconditions.checkBits8;

import java.util.Objects;

/**
 * Class Bits, to manipulate binary strings
 * 
 * @author Omid Karimi (273816)
 * @author Gianni Lodetti (275085)
 */
public final class Bits {

    private final static int[] reverse = new int[] {
            , };
    
    private Bits() {
    }
    
    /**
     * returns an int for which only the bit of given index is 1
     * 
     * @param index
     * @return mask
     * @throws IndexOutOfBoundsException
     *             if invalid index
     */
    public static int mask(int index) throws IndexOutOfBoundsException {
        Objects.checkIndex(index, Integer.SIZE);
        int mask = 1 << index;
        return mask;
    }

    /**
     * returns true if there is 1 at given index for given int 
     *      false if it is a 0
     * 
     * @param bits
     * @param index
     * @return true or false
     * @throws IndexOutOfBoundsException
     *             if invalid index
     */
    public static boolean test(int bits, int index) throws IndexOutOfBoundsException {
        Objects.checkIndex(index, Integer.SIZE);
        int temp = bits >> index;
        return (temp & 1) == 1;
    }

    /**
     * returns true if there is 1 at the index of given bit 
     *      false if it is a 0
     * 
     * @param bits
     * @param bit
     * @return true or false
     */
    public static boolean test(int bits, Bit bit) {
        return test(bits, bit.index());
    }

    /**
     * sets the bit of given index of given int to the new value
     * 
     * @param bits
     * @param index
     * @param newValue
     * @return bits, with newValue
     * @throws IndexOutOfBoundsException
     *             if invalid index
     */
    public static int set(int bits, int index, boolean newValue) throws IndexOutOfBoundsException {
        Objects.checkIndex(index, Integer.SIZE);
        return (newValue == test(bits, index)) ? bits : bits ^ mask(index); 
    }

    /**
     * returns value for which the size LSB bits are identical to those of given
     * int bits (all other bits are 0)
     * 
     * @param size
     * @param bits
     * @return wanted LSB bits
     * @throws IllegalArgumentException
     */
    public static int clip(int size, int bits) throws IllegalArgumentException {
        checkArgument(size >= 0 && size <= Integer.SIZE);
        // if size = 32, use mask 0xffffffff
        if (size == Integer.SIZE) {
            return bits & 0xFFFFFFFF;
        }
        return bits & mask(size) - 1;
    }

    /**
     * returns value for which the bits are identical to those of given
     * int bits going from start index (included) to start + size (excluded)
     * 
     * @param bits
     * @param start
     * @param size
     * @return wanted bits
     * @throws IndexOutOfBoundsException
     */
    public static int extract(int bits, int start, int size) throws IndexOutOfBoundsException {
        Objects.checkFromIndexSize(start, size, Integer.SIZE);
        return clip(size, bits >> start);
    }

    /**
     * returns rotated sequence of size LSB bits of given int bits
     * 
     * @param size
     * @param bits
     * @param distance
     * @return rotated bits
     * @throws IllegalArgumentException
     */
    public static int rotate(int size, int bits, int distance) throws IllegalArgumentException {
        checkArgument(size > 0 && size <= Integer.SIZE);
        distance = Math.floorMod(distance, size); // calculate positive distance
        bits = clip(size, bits); // remove unwanted bits
        // rotate with two shifts
        int leftBits = bits << distance;
        int rightBits = bits >>> (size - distance);
        // bit to bit "or" to get wanted rotated sequence
        return clip(size, leftBits | rightBits);
    }

    /**
     * extends the sign of the received 8 bits value
     * 
     * @param b
     * @return extended sign value
     * @throws IllegalArgumentException
     */
    public static int signExtend8(int b) throws IllegalArgumentException {
        b = checkBits8(b);
        int temp = (byte) b;
        return (int) temp;
    }

    /**
     * reverses 8 LSB bits of given value
     * 
     * @param b
     * @return reversed value for 8 LSB bits
     * @throws IllegalArgumentException
     */
    public static int reverse8(int b) throws IllegalArgumentException {
        b = checkBits8(b);
        return reverse[b];
    }
    
    /**
     * returns same value, but with the 8 LSB bits complemented
     * 
     * @param b
     * @return inverted value for 8 LSB bits
     * @throws IllegalArgumentException
     */
    public static int complement8(int b) throws IllegalArgumentException {
        b = checkBits8(b);
        return clip(8, ~b);
    }

    /**
     * computes a 16 bit value with the 8 MSB being those of highB , and the 8
     * LSB being those of lowB value
     * 
     * @param highB
     * @param lowB
     * @return wanted 16 bits value
     * @throws IllegalArgumentException
     */
    public static int make16(int highB, int lowB) throws IllegalArgumentException {
        highB = checkBits8(highB);
        lowB = checkBits8(lowB);
        highB = highB << 8;
        return highB | lowB;
    }
}
